...
char **strtow(char *str)
{
	...
	for (i = 0, wordlen = 0; i < wc; i++)
	{
		...
		wordlen = word_counter(str, i + 1, 0);
		if (i == 0 && str[i] != ' ')
			wordlen++;
		...
		getfirstchar = word_counter(str, i + 1, 1);
		if (str[0] != ' ' && i > 0)
			getfirstchar++;
		else if (str[0] == ' ')
			getfirstchar++;
